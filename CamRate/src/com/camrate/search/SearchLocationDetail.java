...
public class SearchLocationDetail extends Activity {
	...
	String NewestParse, SelectedLocation, SelectedCount, SelectedSearch;
	RelativeLayout.LayoutParams paramImage, paramImage2;
	ArrayList<HashMap<String, Object>> LocationPost = new ArrayList<HashMap<String, Object>>();
	ArrayList<HashMap<String, String>> LocationPostCommentsData = new ArrayList<HashMap<String, String>>();
	ArrayList<HashMap<String, String>> LocationPostRateData = new ArrayList<HashMap<String, String>>();
	ImageAdapter objImageAdapter;
	// AbsListView listView;
	ImageLoader imageLoader = ImageLoader.getInstance();
	private static final String TAG = SearchLocationDetail.class.getSimpleName();
	...
	public void init() {
		...
		objImageAdapter = new ImageAdapter(LocationPost);
		mPullRefreshGridView = (PullToRefreshGridView) findViewById(R.id.gridview);
		gridView = mPullRefreshGridView.getRefreshableView();
		imageLoader.init(ImageLoaderConfiguration.createDefault(SearchLocationDetail.this));
		...
		mPullRefreshGridView.setOnRefreshListener(new OnRefreshListener2<GridView>() {
			...
			public void onPullDownToRefresh(PullToRefreshBase<GridView> refreshView) {
				...
				if (chkNet.isNetworkConnected()) {
					...
					params.put("api", "LocationDetail2");
		...
		gridView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				Intent intent = new Intent(SearchLocationDetail.this, PostDetail.class);
				intent.putExtra("PostData", LocationPost);
				intent.putExtra("CheckedIndex", position);
				intent.putExtra("SelectedPostData", LocationPost.get(position));
				intent.putExtra("TotalCheckedIndex", LocationPost.size());
	...
	public class GetTagData extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			...
			String url = c.GetBaseUrl() + "api=LocationDetail2&Location=" + SelectedLocation + "&start=" + startIndex + "&end=15&totalrec=15&User_ID=" + User_ID;
			url = url.replace(" ", "%20");
			Log.d("Tag", "url " + url);
		...
		protected void onPostExecute(String jsonString) {
			...
			setArrayList(jsonString);

		}
	}

	public void setArrayList(String NewestParse) {
		mPullRefreshGridView.onRefreshComplete();
		try {
			...
			if (jsonMyFeed.length() > 0) {
				...
				for (int s = 0; s < jsonMyFeed.length(); s++) {
					...
					LocationPost.add(map);
				}
				Log.d("TAG", "LocationPost " + LocationPost.size());
			}
